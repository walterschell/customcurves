...
type p256TestVector struct {
	k *big.Int
	x *big.Int
	y *big.Int
}

func p256TestVectors() []p256TestVector {
	vectors := make([]p256TestVector, 0)
	lines := bytes.Split([]byte(rawP256TestVectors), []byte{'\n'})
	lines = lines[3:] // skip first three lines
	var workingVector p256TestVector
	for _, line := range lines {
		...
		if len(parts) != 2 {
			panic(fmt.Sprintf("invalid test vector line >> %s", line))
		}
		...
		case "k":
			k := new(big.Int)
			_, ok := k.SetString(value, 10)
			if !ok {
				panic(fmt.Sprintf("invalid k >> %v", value))
			}
			workingVector.k = k
		case "x":
			x := new(big.Int)
			_, ok := x.SetString(value, 16)
			if !ok {
				panic(fmt.Sprintf("invalid x >> %v", value))
			}
			workingVector.x = x
		case "y":
			y := new(big.Int)
			_, ok := y.SetString(value, 16)
			if !ok {
				panic(fmt.Sprintf("invalid y >> %v", value))
			}
			workingVector.y = y
			// completed vector
			vectors = append(vectors, workingVector)
			workingVector = p256TestVector{}
		default:
			panic(fmt.Sprintf("invalid test vector key >> %v", key))
		}
	}
	return vectors
}

func TestP256TestVectors(t *testing.T) {
	p256 := P256Curve()
	vectors := p256TestVectors()
	for _, vector := range vectors {
		t.Logf("P256: ScalarBaseMult(%v)\n", vector.k)
		pt := p256.G().Mul(vector.k)
		if pt.X().Cmp(vector.x) != 0 || pt.Y().Cmp(vector.y) != 0 {
			t.Errorf("P256: ScalarBaseMult(%s) = (%s, %s), want (%s, %s)", vector.k.Text(10), pt.X().Text(16), pt.Y().Text(16), vector.x.Text(16), vector.y.Text(16))
		}
	}
}

func testAgreement(t *testing.T, curve *Curve) {
	A, a, err := curve.newKeyPair()
	if err != nil {
		t.Fatalf("NewKeyPair failed")
	}
	B, b, err := curve.newKeyPair()
	if err != nil {
		t.Fatalf("NewKeyPair failed")
	}
...
func TestTinyAgreement(t *testing.T) {
	CTinyParams := &CurveParams{
		a:    big.NewInt(63),
		b:    big.NewInt(33),
		p:    big.NewInt(103),
		n:    big.NewInt(97),
	...
	testAgreement(t, curve)
}

func TestAgreement(t *testing.T) {
	p256 := P256Curve()
	A, a, err := p256.newKeyPair()
	if err != nil {
		t.Fatalf("P256: NewKeyPair failed")
	}
	B, b, err := p256.newKeyPair()
	if err != nil {
		t.Fatalf("P256: NewKeyPair failed")
	}
...
func testDoubling(t *testing.T, curve *Curve) {
	G := curve.G()
	// 2G
	twoG := G.Mul(big.NewInt(2))

	GandG := G.Add(G)
	if !twoG.Equal(GandG) {
		t.Errorf("Doubling failed: 2G (%v) != G+G (%v)", twoG, GandG)
	}
}

func TestTinyDoubling(t *testing.T) {
	CTinyParams := &CurveParams{
		a:    big.NewInt(63),
		b:    big.NewInt(33),
		p:    big.NewInt(103),
...
func TestSub(t *testing.T) {
	...
	G2Expected := G3.Sub(G1)

	if !G2.Equal(G2Expected) {
		t.Fatalf("Subtraction did not work as expected")
	}

}

func TestCurveParametersSerialization(t *testing.T) {
	params := P256Params()
	t.Logf("Curve parameters: %v\n", params)
	writer := new(bytes.Buffer)
	err := params.Write(writer)
	if err != nil {
		t.Fatalf("Could not serialize curve parameters: %v", err)
	}
	serialized := writer.Bytes()
	reader := bytes.NewReader(serialized)
	deserialized, err := ReadCurveParams(reader)
	if err != nil {
		t.Fatalf("Could not deserialize curve parameters: %v", err)
	}
	t.Logf("Deserialized curve parameters: %v\n", deserialized)
	if !params.Equal(deserialized) {
		t.Fatalf("Deserialized curve parameters do not match original")
	}
}
...
func TestEncoding(t *testing.T) {
	...
	serializedScalar, err := scalar.MarshalBinary()
	if err != nil {
	...
	if err != nil {
		t.Fatalf("Could not deserialize public key: %v", err)
	}
	deserializedScalar, err := curve.UnmarshalScalar(serializedScalar)
	if err != nil {
